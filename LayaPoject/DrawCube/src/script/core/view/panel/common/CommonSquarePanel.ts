...
export class CommonSquarePanel extends Laya.Sprite3D implements SpriteInterface {
    ...
    //原始二维坐标系
    private posA = new Laya.Vector3(750, 830, 0);
    //转换后输出的三维坐标系
    ...
    public initView(psoType: number, cameraP: Laya.Camera): void {
        ...
        // this.threeScene.addChild(this.lineSprite3D);
        // var boxLineSprite3D = this.lineSprite3D.addChild(new Laya.PixelLineSprite3D(3500)) as Laya.PixelLineSprite3D;
    ...
    public setSpriteSize(): void {
        let typeNum: number = DrawScript.getInstance().getCurCheckBtn();
        switch (typeNum) {
            case 0:
    ...
    public createSquaPanel(long = 1, width = 1): Laya.Mesh {
